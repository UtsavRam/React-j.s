...
function Home() {    
    ...
    const [Serch,setSerch]=useState('');
        ...
        setInterval(() => {
            const getvalue = JSON.parse(localStorage.getItem('productvalue'));
            setrecentValue(getvalue)
        }, 1000);

        useEffect(()=> {
            const filterdata = products.filter((v)=> (v.title.toLowerCase().includes(recentValue)))
            setSerch(filterdata)
